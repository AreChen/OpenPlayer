...
const pluginSdkGuide = await readFile(new URL("../../../docs/plugins/sdk-1.6-developer-guide.md", import.meta.url), "utf8");
...
assert.match(appSource, /appearance_import_plugin_package/, "frontend must install .opplugin packages through the backend");
assert.match(appSource, /appearance_import_plugin_directory/, "frontend must import plugin directories through the backend");
...
assert.match(appSource, /tasks:\s*Object\.freeze\([\s\S]*start\(input\)[\s\S]*tasks\.start/, "plugin bridge must expose task start");
assert.match(appSource, /tasks:\s*Object\.freeze\([\s\S]*update\(taskId,\s*patch\)[\s\S]*tasks\.update/, "plugin bridge must expose task updates");
assert.match(appSource, /tasks:\s*Object\.freeze\([\s\S]*cancel\(taskId\)[\s\S]*tasks\.cancel/, "plugin bridge must expose task cancellation requests");
assert.match(appSource, /tasks:\s*Object\.freeze\([\s\S]*markCancelled\(taskId\)[\s\S]*tasks\.markCancelled/, "plugin bridge must expose task cancellation acknowledgement");
assert.match(appSource, /handlePluginTasksRuntimeCommand/, "plugin runtime host must route task lifecycle commands");
assert.match(appSource, /MAX_PLUGIN_TASKS_PER_PLUGIN/, "plugin task registry must bound task count per plugin");
...
assert.match(appSource, /"playlist\.changed"/, "plugin runtime must support playlist changed events for queue-aware panels");
assert.match(appSource, /"recording\.changed"/, "plugin runtime must support recording changed events for capture panels");
assert.match(appHookSource, /broadcastPluginRuntimeEvent\("playlist\.changed"[\s\S]*queue[\s\S]*currentIndex/, "playlist changes must be broadcast with queue context");
assert.match(appHookSource, /broadcastPluginRuntimeEvent\("recording\.changed"[\s\S]*recordingState/, "recording state changes must be broadcast to subscribed plugins");
...
assert.match(tauriRuntimeSource, /mpv_embed_read_generated_subtitle/, "mpv runtime must register generated subtitle readback");
...
assert.match(tauriRuntimeSource, /read_generated_subtitle_file/, "generated subtitle readback must read only managed subtitle files");
...
assert.match(appSource, /"media\.segmentExport"/, "plugin bridge must advertise host-mediated media segment export");
assert.match(appSource, /exportSegment\(args[\s\S]*media\.exportSegment/, "plugin bridge must expose a high-level media segment exporter");
assert.match(appSource, /permissions\.has\("media\.export"\)/, "media segment export must require the media.export plugin permission");
assert.match(tauriRuntimeSource, /mpv_embed_export_media_segment/, "mpv runtime must register media segment export");
assert.match(tauriRuntimeSource, /Downloads[\s\S]*Exports|download_dir\(\)[\s\S]*Exports/, "media segment exports must default to a user-visible Downloads export directory");
assert.match(tauriRuntimeSource, /normalizes_media_segment_export_requests_for_audio_and_video[\s\S]*mp3[\s\S]*mkv/, "media segment export format validation must have regression coverage");
...
assert.match(appSource, /\.op-button--danger[\s\S]*--op-danger/, "plugin UI Kit must include a standard danger button variant");
assert.match(appSource, /\.op-switch[\s\S]*\.op-switch__thumb/, "plugin UI Kit must include a theme-aware switch control");
assert.match(appSource, /\.op-slider/, "plugin UI Kit must include a standard range slider class");
assert.match(appSource, /\.op-table[\s\S]*\.op-kbd/, "plugin UI Kit must include dense table and keyboard hint classes");
...
assert.match(appSource, /<iframe[\s\S]*allowTransparency=\{true\}[\s\S]*style=\{\{ backgroundColor: "transparent" \}\}/, "plugin custom view iframes must explicitly allow transparent composition over the video surface");
assert.match(appSource, /resolvePluginViewFrameOpacity[\s\S]*frameOpacitySetting/, "plugin side panel iframe opacity must be derived from a plugin setting instead of hard-coded CSS");
assert.match(appSource, /type UseAppShellHandlersOptions = \{[^}]*activePluginView/s, "shell pointer handlers must know when a plugin view is active");
assert.match(appSource, /type UseAppShellHandlersOptions = \{[^}]*closePluginView/s, "shell pointer handlers must be able to close plugin side panels from video clicks");
assert.match(appSource, /function handleShellPointerDown[\s\S]*activePluginView[\s\S]*closePluginView\(\)/, "clicking the video surface must dismiss active plugin side panels like other floating menus");
assert.match(appSource, /accentOverride \?\? activeTheme\.tokens\.accent/, "plugin custom views must receive the effective user accent override");
assert.match(tauriRuntimeSource, /unsupported plugin view presentation/, "plugin manifests must validate view presentation modes");
assert.match(tauriRuntimeSource, /frame_opacity_setting[\s\S]*number setting/, "plugin manifests must validate setting-backed side panel frame opacity");
assert.match(appSource, /tv:\s*"/, "plugin action icons must include a small TV glyph for TV-like plugins");
assert.match(appSource, /\|\s*"tv"/, "frontend plugin action icon typing must accept tv");
assert.match(appearanceStoreSource, /\|\s*"tv"/, "backend plugin manifest validation must accept tv action icons");
...
assert.match(mpvEmbedSource, /sub-ass-override[\s\S]*force/, "plugin subtitle styling must force ASS override so font, size, color, outline, and shadow changes can take effect");
assert.match(mpvEmbedSource, /set_property\(property[\s\S]*PluginMpvPropertyValue::Number/, "plugin mpv settings must use typed property writes instead of fragile command strings");
...
assert.doesNotMatch(appSource, /<video|videoRef|fileInputRef|type="file"|handleFileInputChange|onDrop=\{handleDrop\}/, "mpv-first player must not keep HTML video or browser File playback");
...
assert.match(appSource, /onPointerMove=\{\(event\) => resizeRegionHandlers\.onPointerMove\(event, region\.direction\)\}/, "resize handles must refresh the native resize cursor on pointer move, not only on pointer enter");
assert.match(appSource, /function\s+handleResizePointerMove\([^)]*direction:\s*ResizeDirection[\s\S]*setNativeResizeCursor\(direction\)/, "resize pointer move must keep the native cursor active while the pointer remains inside a resize hit area");
...
assert.match(appSource, /onPointerMoveCapture[\s\S]*handleResizeSurfacePointerMove/, "overlay shell must refresh the native resize cursor from capture-phase pointer movement across the full transparent overlay");
...
assert.match(appSource, /handleResizePointerEnd[\s\S]*setNativeResizeCursor\(null\)/, "resize cursor must disappear immediately when resize dragging ends instead of falling back to hover state");
assert.doesNotMatch(appSource, /resizeFeedback|ResizeFeedback|resize-feedback|setResizeBoundaryFeedback|clearResizeHoverFeedback|isPointerNearViewportResizeEdge/, "native macOS resize must not keep the temporary in-app resize boundary feedback layer");
...
assert.doesNotMatch(styles, /resize-feedback/, "native macOS resize must not keep temporary visual resize feedback styles");
...
assert.match(mpvEmbedSource, /write_generated_subtitle_file[\s\S]*generated-subtitles/, "mpv embed backend must persist plugin-generated subtitles in a managed app-data directory");
assert.match(mpvEmbedRuntimeSource, /mpv_embed_load_generated_subtitle/, "desktop runtime must register generated subtitle loading for plugin SDK composition");
...
assert.match(appearanceStoreSource, /PLUGIN_INSTALLS/, "appearance store must persist plugin install metadata");
assert.match(appearanceStoreSource, /PLUGIN_PACKAGE_EXTENSION:\s*&str\s*=\s*"opplugin"/, "appearance store must define the OpenPlayer .opplugin package extension");
assert.match(appearanceStoreSource, /zip::ZipArchive/, "appearance store must read .opplugin packages as zip archives");
assert.match(appearanceStoreSource, /extract_plugin_package[\s\S]*enclosed_name/, "plugin package extraction must reject unsafe archive paths");
assert.match(appearanceStoreSource, /appearance_import_plugin_package[\s\S]*appearance_import_plugin_directory[\s\S]*appearance_uninstall_plugin/, "appearance store must expose package install, folder import, and uninstall commands");
assert.match(appearanceStoreSource, /appearance_plugin_runtime_sources/, "appearance store must expose installed plugin runtime sources");
