...
OpenPlayer 1.6.0 将插件体系扩展为面向模块化播放器扩展的文档化 SDK。插件可以组合类型化 manifest、宿主能力检查、运行时事件、隔离存储、受校验的网络请求、支持设置调节侧边栏透明度的半透明主题化自定义视图、原生对话框、片段导出，以及受权限控制的 mpv 播放、滤镜、OSD 和脚本消息能力。

- SDK 指南：[docs/plugins/sdk-1.6-developer-guide.md](docs/plugins/sdk-1.6-developer-guide.md)
