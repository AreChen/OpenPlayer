...
[下载安装包](https://github.com/AreChen/OpenPlayer/releases/latest) · [发布说明](docs/releases/v1.5.1.md) · [插件 SDK](docs/plugins/sdk-1.5-developer-guide.md) · [许可证](LICENSE)
...
OpenPlayer 1.5.1 将插件体系扩展为面向外部开发者和 AI 辅助开发的文档化 SDK。插件可以使用类型化 manifest、宿主能力检查、运行时事件、隔离存储、受校验的网络请求、支持设置调节侧边栏透明度的半透明主题化自定义视图、原生对话框，以及受权限控制的 mpv 播放、滤镜、OSD 和脚本消息能力。
...
- 🐧 Linux：`OpenPlayer_1.5.1_amd64.deb` 和 `OpenPlayer_1.5.1_amd64.AppImage`
- 🍎 macOS：`OpenPlayer_1.5.1_arm64.dmg` 和 `OpenPlayer_1.5.1_x64.dmg`
